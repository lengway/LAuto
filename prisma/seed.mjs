...
async function upsertCar({ brandId, model, generation, years }) {
  ...
  return prisma.car.upsert({
    where: { slug },
    update: { brandId, model, generation, years },
    create: { brandId, model, slug, generation, years },
  });
}

async function upsertPart({ oemNumber, title, categoryId, description, priceFrom, inStock, imageUrl, compatibleCarIds }) {
  ...
  await prisma.partCompatibility.createMany({
    data: compatibleCarIds.map((carId) => ({ partId: part.id, carId })),
    skipDuplicates: true,
  });
...
async function main() {
  ...
  await prisma.vinPattern.upsert({
    where: { pattern: "LVT" },
    update: { carId: tiggo7Pro.id },
    create: { pattern: "LVT", carId: tiggo7Pro.id },
  });

  await prisma.vinPattern.upsert({
    where: { pattern: "L6T" },
    update: { carId: coolray.id },
    create: { pattern: "L6T", carId: coolray.id },
  });

  await prisma.vinPattern.upsert({
    where: { pattern: "LGW" },
    update: { carId: jolion.id },
    create: { pattern: "LGW", carId: jolion.id },
  });

  await prisma.vinPattern.upsert({
    where: { pattern: "LS5" },
    update: { carId: cs55Plus.id },
    create: { pattern: "LS5", carId: cs55Plus.id },
  });

  await prisma.vinPattern.upsert({
    where: { pattern: "LJ1" },
    update: { carId: js6.id },
    create: { pattern: "LJ1", carId: js6.id },
  });
  ...
  await prisma.importJob.create({
    data: {
      file: "seed-initial-catalog.xlsx",
